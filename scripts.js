...
(() => {
  ...
  (() => {
    ...
    /* ─────────────── SCENE 0 · NEURAL NETWORK (MLP) ─────────────── */
    /* Multilayer perceptron — input → 2 hidden → output, fully connected.
       Layers are stacked along X with vertical spread on Y, slight Z jitter. */
    const NN = { layers: [4, 6, 6, 3], xSpan: 280, ySpan: 220, zJit: 24 };

    function setNeuralNet() {
      edges = [];
      let p = 0;
      const layerIdx = [];
      const L = NN.layers.length;

      for (let li = 0; li < L; li++) {
        const count = NN.layers[li];
        const lx = -NN.xSpan / 2 + (li / (L - 1)) * NN.xSpan;
        const ids = [];
        for (let ni = 0; ni < count; ni++) {
          if (p >= N - 12) break;
          const ly = (count === 1 ? 0 : -NN.ySpan / 2 + (ni / (count - 1)) * NN.ySpan);
          const lz = (Math.random() - 0.5) * NN.zJit;
          const pt = particles[p];
          pt.tx = lx; pt.ty = ly; pt.tz = lz;
          pt.role = 'nn-node';
          /* Input layer terra, hidden ink, output amber */
          pt.color = li === 0 ? TERRA : (li === L - 1 ? AMBER : INK2);
          pt.size = li === 0 || li === L - 1 ? 3.2 : 2.8;
          ids.push(p); p++;
        }
        layerIdx.push(ids);
      }

      /* Fully-connected weights between consecutive layers */
      for (let li = 0; li < L - 1; li++) {
        const a = layerIdx[li], b = layerIdx[li + 1];
        for (let i = 0; i < a.length; i++) {
          for (let j = 0; j < b.length; j++) {
            /* Weight strength varies for visual interest */
            const w = 0.25 + ((i * 17 + j * 13 + li * 7) % 100) / 200;
            edges.push([a[i], b[j], w]);
          }
        }
      }

      /* Ambient particles fill the surrounding field */
      while (p < N) {
