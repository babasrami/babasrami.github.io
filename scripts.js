...
(() => {
  ...
  if (!IS_TOUCH && !PREFERS_REDUCED) {
    ...
    const interactive = 'a, button, .stat, .chip, .scene-nav, .work-card, .edu-card, .lang, .contact-link, .nav-logo';
  ...
  (() => {
    ...
    const words = [
      ...
      'Cancer Genomics', 'CNN', 'RNA-seq', 'Survival Analysis', 'Microfluidics',
      'Python', 'TensorFlow', 'scRNA-seq', 'Variant Calling', 'ECM Proteases'
    ];
    const html = [...words, ...words, ...words]
      .map((w) => `<span class="marquee-item">${w}</span>`)
      .join('');
  ...
  /* ─────────────────────────────────────────────────────────────
     SCENE NAV — auto-cycles every 2 s; arrows allow manual jump
     ───────────────────────────────────────────────────────────── */
  let cycleTimer = null;
  ...
  document.getElementById('scene-prev')?.addEventListener('click', () => {
    const prev = (activeScene - 1 + SCENES.length) % SCENES.length;
    goScene(prev);
    startCycle(); // reset timer so it doesn't jump immediately after manual click
  });
