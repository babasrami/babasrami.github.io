...
(() => {
  ...
  (() => {
    ...
    const TERRA = [255, 93, 59];
    const AMBER = [255, 180, 61];
    const SAGE  = [0, 224, 138];
    const SLATE = [77, 141, 255];
    const INK   = [15, 13, 10];
    const INK2  = [42, 36, 28];
  ...
  /* Apply institution logo URLs to .edu-mark--logo elements via CSS var.
     We can't use attr() for url() across all browsers yet, so set --logo
     here from the data-logo attribute. */
  document.querySelectorAll('.edu-mark--logo[data-logo]').forEach((el) => {
    el.style.setProperty('--logo', `url("${el.dataset.logo}")`);
  });
  ...
  (() => {
    ...
    const COLORS = ['#ff5d3b', '#00e08a', '#4d8dff', '#ffb43d'];
  ...
  document.querySelectorAll('[data-carousel]').forEach((wrap) => {
    ...
    /* Source markup is newest-first; keep that order so the carousel
       starts at the most recent role / degree and walks backward through
       the user's history. */
    ...
    /* Measure the scrollable range. Finite: starts at the first card
       centered, ends with the last card centered. Width = distance
    ...
    function measure() {
      ...
      /* getBoundingClientRect respects the current transform; back out
         the live offset so we get untransformed positions. */
      const live = offset || 0;
      maxOffset = Math.max(0, (last.left - first.left));
      /* live is already subtracted via translate; first/last positions
         move together so their delta is invariant. The +live is a no-op. */
      void live;
    }
    ...
    const END_HOLD_MS  = 1600;     /* pause at the end before stopping */

    let offset = 0;
    let auto = !PREFERS_REDUCED;
    let lastUser = -Infinity;
    let endReachedAt = 0;          /* timestamp we first hit the end */
    let isDragging = false;
    let dragStartX = 0, dragStartOff = 0;
    let velocity = 0;
    let inView = true;

    /* Initial offset: 0 means the first card sits centered. */
    ...
    function applyTransforms() {
      /* Clamp offset to [0, maxOffset] — no wrap-around. */
      if (offset < 0) { offset = 0; velocity = 0; }
      if (offset > maxOffset) { offset = maxOffset; velocity = 0; }
      ...
      for (let i = 0; i < all.length; i++) {
        ...
        if (abs < minAbs) { minAbs = abs; focusEl = all[i]; }
      }
    ...
    function frame(now) {
      ...
      const atEnd = offset >= maxOffset - 0.5;
      viewport.classList.toggle('at-end', atEnd);

      if (auto && !isDragging) {
        if (atEnd) {
          /* Hit the last card — pause briefly, then stop auto-advance.
             User must scroll back manually to replay. */
          if (!endReachedAt) endReachedAt = now;
          if (now - endReachedAt > END_HOLD_MS) auto = false;
        } else {
          offset += SPEED * (dt / 16);
          endReachedAt = 0;
        }
      ...
      /* Resume auto-scroll only if the user has scrolled back from the end. */
      if (!isDragging && !auto && (now - lastUser) > RESUME_AFTER && !atEnd) {
        auto = true;
        endReachedAt = 0;
      }
    ...
    window.addEventListener('resize', () => {
      const prevMax = maxOffset;
      measure();
      /* Keep visual position roughly stable across resize */
      if (prevMax) offset = Math.min(maxOffset, (offset / prevMax) * maxOffset);
    });
  });
